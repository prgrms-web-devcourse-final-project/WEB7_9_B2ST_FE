...
export default function BookingSeats({ params }: { params: Promise<{ id: string }> }) {
        ...
        if (response.data) {
          ...
          if (section) {
            filteredSeats = response.data.filter(
              (seat) => seat.sectionName?.toLowerCase() === section.toLowerCase()
            );
          }
  ...
  const getSeatColor = (seat: ScheduleSeatViewRes) => {
    ...
    if (seat.status === 'SOLD') return 'bg-gray-400 cursor-not-allowed border-gray-400';
    if (seat.status === 'HOLD') return 'bg-yellow-300 cursor-not-allowed border-yellow-300';
              ...
              <div className="mb-6">
                ...
                {Object.keys(groupedSeats).map((sectionName) => (
                  <div key={sectionName} className="mb-8">
                    <h3 className="text-lg font-bold text-gray-900 mb-4">{sectionName}구역</h3>
                            ...
                            <div className="flex gap-1 flex-1 flex-wrap">
                              {groupedSeats[sectionName][rowLabel].map((seat) => (
                                <button
                                  ...
                                  title={`${sectionName}구역 ${rowLabel}열 ${seat.seatNumber}번 - ${seat.status === 'AVAILABLE' ? '선택 가능' : seat.status === 'HOLD' ? '예약 중' : '판매 완료'}`}
                                >
                                  {seat.seatNumber}
                                </button>
                              ))}
              ...
              {selectedSeats.length > 0 ? (
                <div className="space-y-4">
                  ...
                  <div className="border-t pt-4">
                    ...
                    <Link
                      href={`/performance/${id}/booking/payment?scheduleId=${scheduleId}&seats=${selectedSeats.map(s => s.scheduleSeatId).join(',')}`}
