...
export default function TradeDetailPage({
  ...
  const [schedule, setSchedule] = useState<PerformanceScheduleListRes | null>(
    null
  );
  const [currentUserId, setCurrentUserId] = useState<number | null>(null);
  const [myTradeRequest, setMyTradeRequest] = useState<{
      ...
      try {
        ...
        if (response.data) {
          ...
          if (response.data.performanceId && response.data.scheduleId) {
            try {
              const schedulesResponse = await performanceApi.getSchedules(
                response.data.performanceId
              );
            ...
            try {
              ...
              if (myRequestsResponse.data) {
                ...
                if (myRequest && myRequest.tradeRequestId) {
                  setMyTradeRequest({
                    tradeRequestId: myRequest.tradeRequestId,
                  });
                }
              }
            } catch (err) {
  ...
  const handlePurchase = async () => {
    ...
    // 결제 페이지로 이동
    router.push(`/trade/${tradeId}/payment`);
  };
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        await tradeApi.updateTradePrice(
                          tradeId,
                          parseInt(newPrice)
                        );
                        alert("가격이 수정되었습니다.");
                        ...
                        // 거래 상세 정보 다시 불러오기
                        const tradeResponse = await tradeApi.getTradeDetail(
                          tradeId
                        );
